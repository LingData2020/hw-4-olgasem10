...
```{r}
sprintf('Amount of participants: %s', length(unique(mono_socio[["ParticipantID"]])))

gender <- mono_socio %>% filter(QuestionCategory=="sex")%>% count(Responce)
sprintf('There are %s males and %s females', gender$n[[2]], gender$n[[1]])

edu <- mono_socio %>% filter(QuestionCategory=="education") %>% distinct(Responce) 
levels <- paste( unlist(c(edu[,1])), collapse=', ')
sprintf('There are following education levels in df: %s', levels)

edu2 <- mono_socio %>% filter(QuestionCategory=="education") %>% count(Responce)
sprintf('%s participants of  %s level, %s of %s, %s of %s and %s of %s', edu2$n[[1]], edu2$Responce[[1]],
        edu2$n[[2]], edu2$Responce[[2]], edu2$n[[3]], edu2$Responce[[3]], edu2$n[[4]], edu2$Responce[[4]])

hand <- mono_socio %>% filter(QuestionCategory=="handedness") %>% count(Responce)
sprintf('There are %s left-randed and %s right-randed participants', hand$n[[1]], hand$n[[2]])
```
...
mono %>%
  ...
  labs(title = "Reaction time density plot",
       x = "Reaction time")
```
...
```
No, we can't say that RT approximately follows normal distribution. It's not symmetric about the mean, has long right tail because of outliers.
```
...
mono %>%
  ggplot(aes(RTlog))+
  geom_density(alpha = 0.4)+
  geom_vline(aes(xintercept=mean(RTlog, na.rm=T)),
               color="red", linetype="dashed", size=1)+
  labs(title = "Logarithmic reaction time density plot",
       x = "Logarithmic reaction time")
...
```
RTlog almost follows normal distribution, but it's still asymmetric, has skewness.
```
...
```{r}
# hint: sd()
sprintf('min: %s', min(mono$RTlog))
sprintf('max: %s', max(mono$RTlog))
sprintf('mean: %s', mean(mono$RTlog))
sprintf('median: %s', median(mono$RTlog))
sprintf('sd: %s', sd(mono$RTlog))
...
```{r}
ggplot(mono1, aes(x = AcceptabilityJ, y = RTlog))+
  geom_violin(aes(color = WordOrder), position = position_dodge(1) )+
  geom_boxplot(aes(color = WordOrder), width = 0.25, position = position_dodge(1))+
  labs(x = "Acceptability Judgement")
...
mono_sum <- mono1 %>%
  ...
  dplyr::summarize(Mean = mean(RTlog, na.rm=TRUE), Sd = sd(RTlog, na.rm=TRUE))
```

```{r}
ggplot(mono_sum, aes(x = AcceptabilityJ, y = Mean, group=WordOrder, color = WordOrder)) + 
...
```
Yes, because p-value = 0.009152, which is less than 5%.
```

What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now?
```
According to this test we can claim that they differ enough to reject null hypothesis.
```

How can you interpret difference with the result of 2.3?
```
Paired t-test result differs from two-sample t-test result because person-to-person variability influences too much to our test in case of two-sample t-test. But that variability is not a objective of our study and paired t-test allows as to reduce its impact and focus on the effect that we are interested in.
```
...
t.test(mean_rtlog$Congruent, mean_rtlog$Incongruent, alternative = "less", var.equal = FALSE, paired = TRUE)
```
...
```
According to the test p-value = 0.0125 which is less than 5%. It means that we should reject our null hypothesis and claim that there is statistically significant difference in logarithm of reaction time for different conditions.
```
...
```
```
 p-value for pair SubjectiveComment-Material-Shape-Color is less than 5% so it has statistically significant difference between logarithms of reaction time.
```
